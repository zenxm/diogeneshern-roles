;(function () {
   ...
   * @constructor
   */
  if ('undefined' === typeof RolesPermission) {
    RolesPermission = {}
  }
  ...
  _.extend(RolesPermission, {
    ...
    createRole: function (role) {
      ...
      if (!role || !role.name || !role.permissions
        || 'string' !== typeof role.name
        || 'object' !== typeof role.permissions
     ...
     *
     * @method createRole
     * @param {String} role to update
     * @return {String} id of new role
     */
    updateRole: function (role) {

      try {
        const itemId = role._id;
        Meteor.roles.update(itemId, {$set: role});
        return itemId;
      } catch (e) {
        // (from Meteor accounts-base package, insertUserDoc func)
        // XXX string parsing sucks, maybe
    ...
    getRole: function (itemId) {
      return Meteor.roles.find({_id: itemId});
    },
     ...
     * Add roles to user.
     */
    addRolesToUser: function (users, roles) {
      // use Template pattern to update user roles
      RolesPermission._updateUserRoles(users, roles, RolesPermission._update_$set_fn)
    },
     ...
     */
    removeUsersFromRoles: function (users, roles) {
      var update
     ...
     * @return {Boolean} true if user is in _any_ of the target roles
     */
    userHasPermissions: function (group, permission) {
      ...
      try {
        let roles = user.roles.map((role) =>
          Meteor.roles.find({_id: role}).fetch()
        );

        let permissions = [];
        roles.forEach(function (rol) {
          permissions.push(lodash.findIndex(rol.permissions, function (o) {
            return o.group === group && o.permission.indexOf(permission) >= 0;
          }));
        });

        return (lodash.sum(permissions) > permissions.length * -1)
      } catch (exception) {
     ...
     */
    getUsersInRole: function (role, options) {
      var query,
        roles = role,
        groupQuery
     ...
     *   @param {String} [group]
     */
    _updateUserRoles: function (users, roles, updateFactory) {
      if (!users) throw new Error("Missing 'users' param")
      if (!roles) throw new Error("Missing 'roles' param")
